...
def info_screen(screen, message="INFO", duration=3, blink=True, border=True, description="", 
    ...
    while True:
        ...
        if current_time - last_update >= 0.1:
            ...
            if not blink or visible:
                ...
                buffer.print_at(message, x_pos, y_pos, Screen.COLOUR_BLUE, Screen.A_BOLD)
            visible = not visible
            ...
            if progress and duration not in (0, -1, None):
                ...
                buffer.print_at("[" + "#" * filled + "-" * (progress_width - filled) + "]", 2, popup_height - 2, Screen.COLOUR_BLUE)
            ...
            if countdown and duration not in (0, -1, None):
                ...
                buffer.print_at(countdown_text, popup_width - len(countdown_text) - 1, 1, Screen.COLOUR_BLUE)
...
def run_info_screen(message="INFO", duration=3, blink=True, border=True, description="", 
                    background_color=Screen.COLOUR_BLACK, progress=False, countdown=False, 
                    spinner=False, popup_width=40, popup_height=10, allow_early_exit=True):
    try:
        if sys.stdin.isatty():  # Check if the script is running in a real terminal
            #Hide the Cursor
            print("\033[?25l")
            Screen.wrapper(lambda screen: info_screen(screen, message, duration, blink, border, 
        ...
        print(f"[INFO] {message} (Falling back to text mode)")
        print(str(e))
    finally:
